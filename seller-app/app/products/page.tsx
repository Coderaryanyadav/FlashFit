...
export default function ProductsPage() {
    ...
    const columns: ColumnDef<Product>[] = [
        ...
        },
        {
            ...
            header: "Stock",
            cell: ({ row }) => {
                const stock = row.getValue("stock") as number | Record<string, number>;
                const totalStock = typeof stock === 'number' ? stock : (stock ? Object.values(stock).reduce((a, b) => a + b, 0) : 0);

                return (
                    <div className={`font-bold ${totalStock < 10 ? "text-red-400" : "text-green-400"}`}>
                        {totalStock}
                    </div>
                );
            }
    ...
    useEffect(() => {
        // Real-time listener for products (Filtered by Store)
